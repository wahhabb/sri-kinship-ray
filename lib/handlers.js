...
const mapKEvents = (kevents) => {
  return kevents.map((kevent) => {
    return {
      ...
      begin: kevent.begin.toLocaleString() + " (Your Local Time)",
      end: kevent.end.toLocaleString() + " (Your Local Time)",
...
exports.selectKEvent = async (req, res) => {
  ...
  if (req.cookies.sriuser) {
    try {
      ...
      const kevents = await db.getKEvents(all, false, false, null, forUser);
...
exports.editKEvent = async (req, res) => {
  ...
  if (req.cookies.sriuser) {
    if (req.body.editevent) {
      ...
      try {
        const kevents = await db.getKEvents(false, [], false, req.body.event);
        await db.getEventIssues(kevents);
        var context = {
          editevent: true,
          issueNames: await mapIssueNames(res),
